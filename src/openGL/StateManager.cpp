...
auto StateManager::ApplyDepthState() -> void
{
	...
	GLCall(glDepthFunc(static_cast<GLenum>(m_DepthState.compareFunc)));
	GLCall(glDepthMask(m_DepthState.writeMask ? GL_TRUE : GL_FALSE));
}
...
auto StateManager::ApplyBlendState() -> void
{
	...
	GLCall(glBlendEquation(static_cast<GLenum>(m_BlendState.equation)));
}
...
auto StateManager::ApplyCullFaceState() -> void
{
	...
	GLCall(glCullFace(static_cast<GLenum>(m_CullFaceState.mode)));
	GLCall(glFrontFace(static_cast<GLenum>(m_CullFaceState.frontFace)));
}
...
auto StateManager::ApplyStencilState() -> void
{
	...
	GLCall(glStencilMask(m_StencilState.writeMask));
}
